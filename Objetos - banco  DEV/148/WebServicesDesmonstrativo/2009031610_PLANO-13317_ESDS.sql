--<DS_SCRIPT>
-- DESCRIÇÃO..: Ajustes de multiempresa
...
CREATE OR REPLACE PROCEDURE dbaps.prc_tiss_demonstrativos(P_ID_TISS_MENSAGEM     IN NUMBER
    ...
    <objeto>
     ...
     <alteracao>03/09/2020 17:04h</alteracao>
  ...
  ****************************************************************************/
	FUNCTION gravarRespostaDemPagamento(P_ID_DEM_RETORNO         IN NUMBER
		...
		CURSOR cPgtoPorData(P_CD_MULTI_EMPRESA IN NUMBER
			...
			UNION ALL
      --PRESTADOR QUE NAO SAO REDE PROPRIA - 2018 EM DIANTE
			SELECT PCP.DT_PAGAMENTO
				FROM DBAPS.REPASSE_PRESTADOR     RP
            ,DBAPS.PRESTADOR             P
						,DBAMV.CON_PAG               CP
         ...
         AND P.TP_CREDENCIAMENTO NOT IN ('P')
            ...
            OR nvl(P_DT_PAGAMENTO, '*') <> '*' AND TRUNC(PCP.DT_PAGAMENTO) = P_DT_PAGAMENTO)
			 ...
			 GROUP BY PCP.DT_PAGAMENTO
       UNION ALL
       ----PRESTADOR REDE PROPRIA - 2018 EM DIANTE
  ...
  ****************************************************************************/
	...
	BEGIN
		...
		SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL
			INTO nIdTissDemErro
			FROM DUAL;
		nIdTissMensagem := dbaps.FNC_TISS_INSERE_CABECALHO(P_ID_TISS_MENSAGEM, 'DEMONSTRATIVO_PAGAMENTO' -- Teve que se escolher entre pagamento ou analise de conta
																											, 'demonstrativoPagamento', P_NR_REGISTRO_ANS, P_NR_CNPJ_OPERADORA, P_CD_VERSAO, P_CD_PRESTADOR_OPERADORA, P_NR_CNPJ_PRESTADOR, P_NR_CPF_PRESTADOR, P_CD_GLOSA, P_DS_GLOSA, P_DS_GLOSA);
		/**
