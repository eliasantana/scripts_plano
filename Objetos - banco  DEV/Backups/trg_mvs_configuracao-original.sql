...
FOR EACH ROW
  ...
  /**************************************************************
    <objeto>
     ...
     <usuario>Jarbas Junior</usuario>
     <alteracao>07/04/2020 14:15</alteracao>
     <ultimaAlteracao>
       Adicionado validação na chave - CONTMED_REGRA_PRESTADOR_COOP. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_ALTERA_VALOR_COB. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_ALTERA_VALOR_COB_PTU. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_PERMITE_EXCLUIR_CM. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_ACEITA_PERC_PRESTADOR. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_VALOR_PAGO_MAIOR_CM. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_AUDITORIA_LOCAL_INTER. (Jarbas Junior)
       Adicionado validação na chave - CONTMED_ALTERA_VALOR_PAG. (Maikon Oliveira)
       Adicionado validação na chave - CONTMED_ALTERA_VALOR_PAG_PTU. (Maikon Oliveira)
       Adicionado validação na chave - CONTMED_IMPORTA_AUDITOR_AUT.  (Maikon Oliveira)
       Adicionado validação na chave - CONTMED_FATOR_URGENCIA_CBHPM. (Jarbas Junior)
     </ultimaAlteracao>
     <descricao>TRIGGER DE VALIDACAO DOS PARAMETROS GLOBAIS DO SISTEMA</descricao>
     <parametro></parametro>
     <versao>1.45</versao>
     <soul>PLANO-01-144</soul>
    </objeto>
  ***************************************************************/
...
BEGIN
  ...
  vMsgValorVazio := 'Valor da chave não pode ser vazio!';
  ...
  IF Nvl(:NEW.CHAVE, :OLD.CHAVE) = 'ANS_QTD_MESES_ANALISE_RPC' THEN
    ...
    IF To_Number(REGEXP_REPLACE(:NEW.VALOR, '[^0-9]')) > 12 THEN
      Raise_Application_Error(-20001, 'Valor da chave não pode ser menor que 12 meses.');
    END IF;
