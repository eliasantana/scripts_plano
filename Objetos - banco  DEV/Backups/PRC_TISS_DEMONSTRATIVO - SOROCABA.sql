PROMPT CREATE OR REPLACE PROCEDURE prc_tiss_demonstrativos
CREATE OR REPLACE PROCEDURE prc_tiss_demonstrativos ( P_ID_TISS_MENSAGEM IN NUMBER
                                                              ,P_ID_TISS_MENSAGEM_OUT OUT NUMBER)
IS
  /**************************************************************
    <objeto>
     <nome>prc_tiss_demonstrativos</nome>
     <usuario>Maikon Oliveira</usuario>
     <alteracao>26/03/2019</alteracao>
     <ultimaAlteracao>alterações nos cursores para os clientes que utilizam folha de pagamento</ultimaAlteracao>
     <descricao>
  ...
  ***************************************************************/
  ...
  CURSOR cTissMensagem IS
    SELECT TM.ID,
           ...
           NVL(TO_DATE(DT_TRANSACAO || ' ' || SubStr(HR_TRANSACAO, 1, 8), 'YYYY-MM-DD HH24:MI:SS')
          ,SYSDATE) DT_TRANSACAO
          ,TDR.TP_DEMONSTRATIVO
      FROM DBAPS.TISS_MENSAGEM TM, DBAPS.TISS_DEMNSTRV_RETORNO TDR
  ...
  CURSOR cDadosPrestador(PCD_PRESTADOR IN NUMBER
    ...
    WHERE (LPad(NR_CPF_CGC, 14, '0') = NVL(PCD_PRESTADOR, lpad(PNR_CNPJCPF, 14, '0'))
           OR PRESTADOR.CD_INTERNO = to_char(PCD_PRESTADOR)
           OR PRESTADOR.CD_PRESTADOR = PCD_PRESTADOR
           )
      AND TP_SITUACAO = 'A'
  ...
  CURSOR cTabela(P_CD_PROCEDIMENTO IN VARCHAR2) IS
    SELECT CD_TABELA FROM (
      SELECT '18' CD_TABELA FROM DBAPS.TISS_INSTALACAO_TABELA_18 WHERE CD_PROCEDIMENTO = P_CD_PROCEDIMENTO
      UNION
      SELECT '19' CD_TABELA FROM DBAPS.TISS_INSTALACAO_TABELA_19 WHERE CD_PROCEDIMENTO = P_CD_PROCEDIMENTO
      UNION
      SELECT '20' CD_TABELA FROM DBAPS.TISS_INSTALACAO_TABELA_20 WHERE CD_PROCEDIMENTO = P_CD_PROCEDIMENTO
      UNION
      SELECT '22' CD_TABELA FROM DBAPS.TISS_INSTALACAO_TABELA_22 WHERE CD_PROCEDIMENTO = P_CD_PROCEDIMENTO
    );
/**
 ...
 */
 CURSOR cProtocoloCtamed(P_NR_PROTOCOLO IN VARCHAR2) IS
    SELECT  PRO.CD_PROTOCOLO_CTAMED,
            ...
            L.CD_LOTE,
            PRO.ID_TISSLOTGUIA
      FROM DBAPS.PROTOCOLO_CTAMED PRO, DBAPS.PRESTADOR P, DBAPS.LOTE L
   ...
   CURSOR cProtocolo( P_CD_PROTOCOLO IN VARCHAR
     ...
     SELECT FAT.CD_FATURA CD_FATURA
            ...
            ,(SELECT SUM(VCM.VL_TOTAL_PAGO) VL_TOTAL_PAGO
                FROM DBAPS.V_CTAS_MEDICAS VCM, DBAPS.LOTE LOT
                WHERE VCM.CD_LOTE = LOT.CD_LOTE
                    AND LOT.cd_protocolo_ctamed = TO_NUMBER(P_CD_PROTOCOLO)
                    AND DBAPS.fnc_mvs_retorna_prestador_rep
                      (VCM.cd_prestador_principal , Nvl( VCM. cd_prestador , VCM .cd_prestador_principal )) = P_CD_PRESTADOR_PRINCIPAL
                     ) VL_TOTAL_PAGO
 ...
 */
  CURSOR cProtocoloCmGuia ( P_CD_PROTOCOLO IN VARCHAR ,P_CD_PRESTADOR_PRINCIPAL IN NUMBER) IS
        ...
        FROM (
              SELECT   CTA.CD_GUIA_EXTERNA NR_GUIA_PRESTADOR
                      ...
                      ,NVL((SELECT 'S' FROM DBAPS.V_CTAS_MEDICAS V WHERE V.CD_CONTA_MEDICA = CTA.CD_CONTA_MEDICA
                      ...
                      ,(SELECT SUM(VCM.VL_TOTAL_PAGO) VL_TOTAL_PAGO
                          ...
                          WHERE  VCM.CD_LOTE = LOT.CD_LOTE
                            AND LOT.CD_PROTOCOLO_CTAMED = TO_NUMBER(P_CD_PROTOCOLO)
                            AND DBAPS.FNC_MVS_RETORNA_PRESTADOR_REP (VCM.CD_PRESTADOR_PRINCIPAL , Nvl( VCM.CD_PRESTADOR , VCM.CD_PRESTADOR_PRINCIPAL )) = P_CD_PRESTADOR_PRINCIPAL
                            AND (VCM.CD_GUIA_EXTERNA = CTA.CD_GUIA_EXTERNA OR VCM.NR_GUIA = CTA.NR_GUIA)) VL_LIBERADO
  ...
  */
  CURSOR cProtocoloCmGuiaProcedimento (P_CD_PROTOCOLO IN VARCHAR, P_NR_GUIA IN NUMBER, P_CD_PRESTADOR_PRINCIPAL IN NUMBER) IS
    ...
    WHERE  LOT.CD_PROTOCOLO_CTAMED = PRO.CD_PROTOCOLO_CTAMED
      ...
      AND  (CTA.NR_GUIA = P_NR_GUIA OR CTA.CD_GUIA_EXTERNA = To_Char(P_NR_GUIA))
      -- apenas pagamentos feitos para o prestador que solicitou a mensagem no tiss serão exibidos
      AND  DBAPS.fnc_mvs_retorna_prestador_rep(CTA.cd_prestador_principal , Nvl( CTA. cd_prestador , CTA.cd_prestador_principal )) = P_CD_PRESTADOR_PRINCIPAL
      AND  CTA.TP_SITUACAO NOT IN ('NA', 'GA') --apenas itens auditados
  ...
  BEGIN /* BLOCO DA FUNÇÃO ****************************************************/
    OPEN  cSolicitacao (P_ID_DEM_RETORNO);
    FETCH cSolicitacao INTO rSolicitacao;
    CLOSE cSolicitacao;
    ...
    SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemAna FROM SYS.DUAL;
    ...
    ELSE
      ...
      bPrestadorValido := dbaps.FNC_VALIDA_PRESTADOR(rSolicitacao.cd_prestador_contratado
                                                       ,Nvl(rSolicitacao.NR_CNPJ_CONTRATADO, rSolicitacao.NR_CPF_CONTRATADO)
      ...
      IF vCdGlosa IS NULL THEN
        BEGIN
          ...
          nDsPrestador := NULL;
          nCdCnes      := NULL;
          FOR rDadosPrestador IN cDadosPrestador(rSolicitacao.CD_PRESTADOR_CONTRATADO
                                                ,NVL(rSolicitacao.nr_cnpj_contratado, rSolicitacao.nr_cpf_contratado)
                                                ,P_CD_MULTI_EMPRESA)
          LOOP
      ...
      ELSE
        ...
        -- buscando solicitações de protocolos (no máximo 30)
        FOR rSolicitacaoProtocolo IN cSolicitacaoProtocolo(P_ID_DEM_RETORNO)
        LOOP
          /* Obtém dados do lote/protocolo em contas médicas ******************/
          SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemAnaFatLot FROM SYS.DUAL;
          -- zerando variaveis
          vCdGlosa               := NULL;
          vDsGlosa               := NULL;
          dDtPagamento           := NULL;
         ...
         FETCH cProtocoloCtamed INTO rProtocoloCtamed;
         ...
         IF vCdGlosa IS NULL AND rProtocoloCtamed.CD_PRESTADOR <> nCdPrestador THEN
           vCdGlosa := '5013';
           vDsGlosa := 'PROTOCOLO NÃO PERTENCE AO PRESTADOR DE CÓDIGO '||nCdPrestador;
           vCdStatusProtocolo := 7; -- 7 - Não Localizado
         END IF;
         CLOSE cProtocoloCtamed;
         -- BUSCA DADOS DO PROTOCOLO (APENAS ITENS AUDITADOS)
         IF vCdGlosa IS NULL AND rProtocoloCtamed.CD_STATUS_PROTOCOLO IN (3,6) THEN -- LIBERADO PARA PAGAMENTO OU PAGAMENTO EFETUADO
          OPEN cProtocolo (rSolicitacaoProtocolo.NR_PROTOCOLO
                          ,nCdPrestador);
          FETCH cProtocolo INTO nCdFatura
                               ,nCdProcoloCtaMed
                               ,nNrLotePrestador
                               ,dDtEnvioLote
          ...
          CLOSE cProtocolo;
          END IF;
          nSumVlInformadoGeral  := nSumVlInformadoGeral + nVlInformadoProtocolo;
          nSumVlProcessadoGeral := nSumVlProcessadoGeral + nVlProcessadoProtocolo;
          nSumVlLiberadoGeral   := nSumVlLiberadoGeral + nVlliberadoProtocolo;
          ...
          IF vCdGlosa IS NULL THEN
            ...
            FOR rProtocoloCMGuia IN cProtocoloCmGuia( nCdProcoloCtaMed, nCdPrestador)
            LOOP
              -- BUSCANDO SEQUENCE PARA GUIAS DO PROTOCOLO
              SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemAnaFatLotGui FROM SYS.DUAL;
              vHrEntrada := null;
              vHrSaida := null;
              BEGIN
              ...
              INSERT INTO DBAPS.TISS_DEM_ANA_FAT_LOT_GUI
                ...
                FOR rProtocoloCmGuiaProcedimento IN cProtocoloCmGuiaProcedimento(nCdProcoloCtaMed, nvl(rProtocoloCMGuia.NR_GUIA_OPERADORA, rProtocoloCMGuia.NR_GUIA_PRESTADOR ), nCdPrestador)
                LOOP
                    -- SEQUENCE
                    SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemAnaFatLotGuiPr FROM SYS.DUAL;
                    -- Grava os dados dos procedimentos da guia
                    vCdTissTabela := NULL;
                    OPEN cTabela(rProtocoloCmGuiaProcedimento.CD_PROCEDIMENTO);
                    FETCH cTabela INTO vCdTissTabela;
                    CLOSE cTabela;
                  ...
                  IF rProtocoloCmGuiaProcedimento.CD_TIPO_GLOSA IS NOT NULL THEN
                    -- SEQUENCE
                    SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemAnaFatLotGuiPrGl FROM SYS.DUAL;
                    -- Grava a glosa do procecimento
                    OPEN cMotivoGlosa(rProtocoloCmGuiaProcedimento.CD_TIPO_GLOSA);
                    FETCH cMotivoGlosa INTO rMotivoGlosa;
                    CLOSE cMotivoGlosa;
        ...
        IF nIdTissDemAna IS NOT NULL THEN
            UPDATE DBAPS.TISS_DEM_ANA SET VL_INFORMADO_GERAL = nSumVlInformadoGeral,
...
SELECT PCP.CD_ITCON_PAG
 ...
 GROUP BY PCP.CD_ITCON_PAG;
      ...
      CURSOR cFormaPgt (P_CD_ITCON_PAG IN NUMBER) IS
        SELECT Decode(PCP.TP_PAGAMENTO,'5', '1'
                                       ,'8', '1'
                                       ,'1', '3'
                                       ,'2', '3'
                                       ,'3', '4'
    ...
    CURSOR cDadosResumoPgt(P_CD_ITCON_PAG IN NUMBER,
                           P_CD_MULTI_EMPRESA IN NUMBER) IS
  ...
  UNION ALL
   ...
   CURSOR cCredDebitoPorData(P_CD_ITCON_PAG IN NUMBER) IS
    ...
    TYPE rcCredDeb IS RECORD (TP_INDICADOR NUMBER, TP_DEBITO_CREDITO VARCHAR2(3), DS_OBSERVACAO VARCHAR2(500), VL_LANCAMENTO NUMBER);
  ...
  BEGIN
    ...
    ELSE
     ...
     IF rSolicitacao.DT_COMPETENCIA IS NOT NULL THEN
       ...
       BEGIN
          SELECT TO_DATE(rSolicitacao.DT_COMPETENCIA||'01', 'YYYYMMDD') INTO dTeste FROM DUAL;
          vDtCompetencia := SubStr(rSolicitacao.DT_COMPETENCIA, 5, 2)||SubStr(rSolicitacao.DT_COMPETENCIA, 1, 4);
       EXCEPTION
        WHEN OTHERS THEN
         vCdGlosa := '5013';
     ...
     END IF;
      ...
      IF vCdGlosa IS NULL THEN
        BEGIN
          ...
          nDsPrestador := NULL;
          nCdCnes      := NULL;
          FOR rPrestador IN cDadosPrestador(rSolicitacao.CD_PRESTADOR_CONTRATADO
                                                ,NVL(rSolicitacao.nr_cnpj_contratado, rSolicitacao.nr_cpf_contratado)
                                                ,P_CD_MULTI_EMPRESA)
          LOOP
            vCount := vCount +1;
      ...
      ELSE -- solicitação está 'OK'
        /* Verificando se sistema possui integração com financeiro *************/
        snIntegraFinanceiro := 'S'; -- default é sim
        SELECT SN_INTEGRA_CON_PAG INTO snIntegraFinanceiro FROM DBAPS.PLANO_DE_SAUDE WHERE ID = 1;
        ...
        i := 0;
        FOR rPgtoPorData IN cPgtoPorData(P_CD_MULTI_EMPRESA
                                            ,nCdPrestador
                                            ,vDtCompetencia
                                            ,dDtPagamento)
        LOOP
          ...
          LOOP
            ...
            FOR rDadosResumoPgt IN cDadosResumoPgt(rItensPagamento.CD_ITCON_PAG,P_CD_MULTI_EMPRESA) LOOP
              SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL
              ...
              INSERT INTO DBAPS.TISS_DEM_PGT_ITEM_RESUMO
                ...
                VALUES
                  ...
                  nTotalInformadoPorData := nTotalInformadoPorData + nvl(rDadosResumoPgt.VL_INFORMADO,0);
                  nTotaLiberadoPorData := nTotaLiberadoPorData + nvl(rDadosResumoPgt.VL_LIBERADO,0);
                  nTotalGlosaPorData := nTotalGlosaPorData + nvl(rDadosResumoPgt.VL_GLOSA,0);
            ...
            FOR rCredDebitoPorData IN cCredDebitoPorData(rItensPagamento.CD_ITCON_PAG) LOOP
                ...
                IF rCredDebitoPorData.TP_INDICADOR = 1 THEN
                    nTotalCreditosPorData := nTotalCreditosPorData + rCredDebitoPorData.VL_LANCAMENTO;
                END IF;
                -- DEBITO
                IF rCredDebitoPorData.TP_INDICADOR = 2 THEN
                    nTotalDebitosPorData := nTotalDebitosPorData + rCredDebitoPorData.VL_LANCAMENTO;
                END IF;
                ...
                VALUES (
                  ...
                  ,nIdTissDemPgtItem
                  ,rCredDebitoPorData.TP_INDICADOR      -- TP_INDICADOR
                  ,rCredDebitoPorData.TP_DEBITO_CREDITO -- TP_DEBITO_CREDITO
                  ,rCredDebitoPorData.DS_OBSERVACAO     -- DS_DB_CR
           ...
           *******************************************************************/
           ...
           nTotalInformadoPorDataDem := nTotalInformadoPorDataDem + NVL(nTotalInformadoPorData,0);
           nTotaLiberadoPorDataDem := nTotaLiberadoPorDataDem + NVL(nTotaLiberadoPorData,0);
           nTotalGlosaPorDataDem := nTotalGlosaPorDataDem + NVL(nTotalGlosaPorData,0);
           -- total geral liquido
           nTotalDebitoPorDataDem := nTotalDebitoPorDataDem + nTotalDebitosPorData;
           nTotalCreditoPorDataDem := nTotalCreditoPorDataDem + nTotalCreditosPorData;
          /* Débitos/Créditos *************************************************/
          --TODO
          END LOOP; --rItensPagamento
        END LOOP; --rPgtoPorData
        ...
        IF nCountPgtPorData = 0 THEN
          ...
          VALUES
                ...
                ,nIdTissMensagem
                ,'5013'
                ,'NÃO FORAM ENCONTRADOS PAGAMENTOS NA '||DECODE(rSolicitacao.DT_COMPETENCIA, NULL, 'DATA DE PAGAMENTO', 'COMPETÊNCIA') ||' FORNECIDA: '
        ...
        ELSE
          ...
          i := 0;
          WHILE (Nvl(aCredDeb(i).TP_INDICADOR,0) <> -1)
          LOOP
              INSERT INTO DBAPS.TISS_DEM_PGT_DEB_CRE
                ...
                VALUES (
                   DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL
                  ,nIdTissDemPgt
                  ,aCredDeb(i).TP_INDICADOR      -- TP_INDICADOR
                  ,aCredDeb(i).TP_DEBITO_CREDITO -- TP_DEBITO_CREDITO
                  ,aCredDeb(i).DS_OBSERVACAO     -- DS_DB_CR
                  ,aCredDeb(i).VL_LANCAMENTO     -- VL_DB_CR
                );
             i := i + 1;
          END LOOP;
          /* totaisLiquidosDemonstrativo TODO */
        END IF; --nCountPgtPorData = 0
      END IF; --vCdGlosa IS NOT NULL
    END IF; -- P_CD_GLOSA_CABECALHO IS NOT NULL
    RETURN nIdTissMensagem;
  END;
/*****************************************************************************
...
BEGIN
  -- BUSCANDO SEQUENCE
  SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemErro FROM DUAL;
  nIdTissMensagem := dbaps.FNC_TISS_INSERE_CABECALHO(P_ID_TISS_MENSAGEM
...
BEGIN
  ...
  dbaps.prc_tiss_valida_cabecalho(P_ID_TISS_MENSAGEM,
                                     nCdPrestadorNaOperadora, -- retorno
  ...
  IF vCdGlosa IS NULL THEN
    ...
    FETCH cWebserviceAtivo INTO rWebserviceAtivo;
    ...
    END IF;
  END IF;
  IF vCdGlosa is null and rTissMensagem.TP_DEMONSTRATIVO IS NULL THEN
      vCdGlosa := '5013';
      vDsGlosa := 'NÃO FOI INFORMADO UM TIPO DE DEMONSTRATIVO NA SOLICITAÇÃO';
  END IF;
  IF rTissMensagem.TP_DEMONSTRATIVO = '1' THEN -- DEMONSTRATIVA DE PAGAMENTO
    P_ID_TISS_MENSAGEM_OUT := gravarRespostaDemPagamento(rTissMensagem.ID_DEM_RETORNO
                                                         ,P_ID_TISS_MENSAGEM
                                                         ,Nvl(nNrRegistroAnsOrigem, nvl(rTissMensagem.NR_REGISTRO_ANS_DESTINO, 999999))
                                                         ,nCnpjOperadora
                                                         ,vDsOperadora
  ...
  ELSIF rTissMensagem.TP_DEMONSTRATIVO = '2' THEN
    P_ID_TISS_MENSAGEM_OUT := gravarRespostaDemAnalise(rTissMensagem.ID_DEM_RETORNO
                                                      ,P_ID_TISS_MENSAGEM
                                                      ,Nvl(nNrRegistroAnsOrigem, nvl(rTissMensagem.NR_REGISTRO_ANS_DESTINO, 999999))
                                                      ,nCnpjOperadora
                                                      ,vDsOperadora
  ...
  ELSIF rTissMensagem.TP_DEMONSTRATIVO = '3' THEN
   P_ID_TISS_MENSAGEM_OUT := gravarRespostaErro(P_ID_TISS_MENSAGEM
                                               ,Nvl(nNrRegistroAnsOrigem, nvl(rTissMensagem.NR_REGISTRO_ANS_DESTINO, 999999))
                                               ,nCnpjOperadora
                                               ,vDsOperadora
                                               ,rTissMensagem.CD_VERSAO
                                               ...
                                               ,nNrCpfPrestador
                                               ,'5013'
                                               ,'DEMONSTRATIVO DE PAGAMENTO - ODONTOLOGIA NÃO ATIVO NO MOMENTO'
                                               ,nCdMultiEmpresaOrigem);
  ELSE
   P_ID_TISS_MENSAGEM_OUT := gravarRespostaErro(P_ID_TISS_MENSAGEM
                                               ,Nvl(nNrRegistroAnsOrigem, nvl(rTissMensagem.NR_REGISTRO_ANS_DESTINO, 999999))
...
EXCEPTION
    ...
    SELECT DBAPS.SEQ_TISS_MENSAGEM.NEXTVAL INTO nIdTissDemAna FROM SYS.DUAL;
    SELECT DBAPS.SEQ_TISS_MENSAGEM_LOG.NEXTVAL INTO nIdTissMensagemLog FROM DUAL;
    ...
    SELECT valor into vVersaoTiss  FROM DBAPS.MVS_CONFIGURACAO
    WHERE  CHAVE = 'VERSAO_TISS'  AND CD_MULTI_EMPRESA = DBAMV.PKG_MV2000.LE_EMPRESA AND ROWNUM =1;
    -- Grava o cabecalho da resposta
    P_ID_TISS_MENSAGEM_OUT := dbaps.FNC_TISS_INSERE_CABECALHO(P_ID_TISS_MENSAGEM,
                                                              ...
                                                              NULL,
                                                              '5013',
                                                              'OCORREU UM ERRO NO PROCESSAMENTO DA SUA REQUISIÇÃO. ERRO CÓD: '||nIdTissMensagemLog,
                                                              'OCORREU UM ERRO NO PROCESSAMENTO DA SUA REQUISIÇÃO. ERRO CÓD: '||nIdTissMensagemLog
                                                             );
    INSERT INTO DBAPS.TISS_DEM_RETORNO_ERRO
      (CD_TISS_DEM_RETORNO_ERRO
...
GRANT EXECUTE ON prc_tiss_demonstrativos TO dbamv;
GRANT EXECUTE ON prc_tiss_demonstrativos TO dbasgu;
GRANT EXECUTE ON prc_tiss_demonstrativos TO mv2000;
GRANT EXECUTE ON prc_tiss_demonstrativos TO mvintegra;
